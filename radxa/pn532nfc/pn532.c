...
uint32_t readRegister(uint16_t reg)
{
    ...
    if (writeCommand(pn532_packetbuffer, 3, NULL, 0)) {
        return 0;
    }
...
bool writeGPIO(uint8_t pinstate)
{
    ...
    printf("Writing P3 GPIO: ");
    printf("%x", pn532_packetbuffer[1]);
    printf("\n");
...
uint8_t readGPIO(void)
{
    ...
    */
    ...
    //printf("P3 GPIO: "); printf_HEX(pn532_packetbuffer[7]);
    //printf("P7 GPIO: "); printf_HEX(pn532_packetbuffer[8]);
    //printf("I0I1 GPIO: "); printf_HEX(pn532_packetbuffer[9]);
    //printf("\n");
...
bool readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout, bool inlist)
{
    ...
    */
    ...
    printf("ATQA: 0x");  printf("%04x\n", sens_res);
    printf("SAK: 0x");  printf("%02x\n", pn532_packetbuffer[4]);
...
uint8_t mifareclassic_AuthenticateBlock (uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
    ...
    //printf("%d %d\n", sizeof(pn532_packetbuffer), _uidLen);
    if (writeCommand(pn532_packetbuffer, 10 + _uidLen, NULL, 0))
        return 0;

    //printf("writeCommand done\n");
...
uint8_t mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t *data)
{
    ...
    /* Send the command */
    if (writeCommand(pn532_packetbuffer, 20, NULL, 0)) {
        return 0;
    }
...
uint8_t mifareultralight_WritePage (uint8_t page, uint8_t *buffer)
{
    ...
    /* Send the command */
    if (writeCommand(pn532_packetbuffer, 8, NULL, 0)) {
        return 0;
    }
...
int8_t tgInitAsTargetx(const uint8_t* command, const uint8_t len, const uint16_t timeout){
  
  int8_t status = writeCommand(command, len, NULL, 0);
    if (status < 0) {
        return -1;
    }
...
int8_t tgInitAsTarget(uint16_t timeout)
{
    ...
    return tgInitAsTargetx(command, sizeof(command), timeout);
}
...
int16_t inRelease(const uint8_t relevantTarget){
    ...
    if (writeCommand(pn532_packetbuffer, 2, NULL, 0)) {
        return 0;
    }
...
int8_t felica_Polling(uint16_t systemCode, uint8_t requestCode, uint8_t * idm, uint8_t * pmm, uint16_t *systemCodeResponse, uint16_t timeout)
{
  ...
  if (pn532_packetbuffer[0] == 0) {
    ...
    printf("Unhandled number of targets inlisted. NbTg: %x\n", pn532_packetbuffer[7]);
    return -3;
  }
...
int8_t felica_SendCommand (const uint8_t *command, uint8_t commandlength, uint8_t *response, uint8_t *responseLength)
{
  ...
  if (writeCommand(pn532_packetbuffer, 3, command, commandlength)) {
    printf("Could not send FeliCa command\n");
    return -2;
  }
  ...
  if ((pn532_packetbuffer[0] & 0x3F)!=0) {
    printf("Status code indicates an error: %x\n", pn532_packetbuffer[0]);
    return -4;
  }
...
int8_t felica_RequestService(uint8_t numNode, uint16_t *nodeCodeList, uint16_t *keyVersions)
{
  ...
  if (felica_SendCommand(cmd, cmdLen, response, &responseLength) != 1) {
    printf("Request Service command failed\n");
    return -2;
  }

  // length check
  if ( responseLength != 10+2*numNode ) {
    printf("Request Service command failed (wrong response length)\n");
    return -3;
  }
...
int8_t felica_RequestResponse(uint8_t * mode)
{
  ...
  if (felica_SendCommand(cmd, 9, response, &responseLength) != 1) {
    printf("Request Response command failed\n");
    return -1;
  }

  // length check
  if ( responseLength != 10) {
    printf("Request Response command failed (wrong response length)\n");
    return -2;
  }
...
int8_t felica_ReadWithoutEncryption (uint8_t numService, const uint16_t *serviceCodeList, uint8_t numBlock, const uint16_t *blockList, uint8_t blockData[][16])
{
  if (numService > FELICA_READ_MAX_SERVICE_NUM) {
    printf("numService is too large\n");
    return -1;
  }
  if (numBlock > FELICA_READ_MAX_BLOCK_NUM) {
    printf("numBlock is too large\n");
    return -2;
  }
  ...
  uint8_t responseLength;
  if (felica_SendCommand(cmd, cmdLen, response, &responseLength) != 1) {
    printf("Read Without Encryption command failed\n");
    return -3;
  }

  // length check
  if ( responseLength != 12+16*numBlock ) {
    printf("Read Without Encryption command failed (wrong response length)\n");
    return -4;
  }

  // status flag check
  if ( response[9] != 0 || response[10] != 0 ) {
    printf("Read Without Encryption command failed (Status Flag: %x%x)\n", pn532_packetbuffer[9], pn532_packetbuffer[10]);
    return -5;
  }
...
int8_t felica_WriteWithoutEncryption (uint8_t numService, const uint16_t *serviceCodeList, uint8_t numBlock, const uint16_t *blockList, uint8_t blockData[][16])
{
  ...
  uint8_t responseLength;
  if (felica_SendCommand(cmd, cmdLen, response, &responseLength) != 1) {
    printf("Write Without Encryption command failed\n");
    return -3;
  }

  // length check
  if ( responseLength != 11 ) {
    printf("Write Without Encryption command failed (wrong response length)\n");
    return -4;
  }

  // status flag check
  if ( response[9] != 0 || response[10] != 0 ) {
    printf("Write Without Encryption command failed (Status Flag: %x%x)\n", pn532_packetbuffer[9], pn532_packetbuffer[10]);
    return -5;
  }
...
int8_t felica_RequestSystemCode(uint8_t * numSystemCode, uint16_t *systemCodeList)
{
  ...
  uint8_t responseLength;
  if (felica_SendCommand(cmd, 9, response, &responseLength) != 1) {
    printf("Request System Code command failed\n");
    return -1;
  }
  *numSystemCode = response[9];
  ...
  if ( responseLength < 10 + 2 * *numSystemCode ) {
    printf("Request System Code command failed (wrong response length)\n");
    return -2;
  }
...
int8_t felica_Release()
{
  ...
  printf("Release all FeliCa target\n");
